    ...
    if ( empty( get_option('atGroupConf') ) ) {
        ...
        foreach (amministrazionetrasparente_getarray_default() as $inner) {
            ...
            foreach ($inner[1] as $value) {
                ...
                $settings_default[ sanitize_title( $inner[0] ) ][] = $term->term_id;
            }
            update_option( 'atGroupConf', $settings_default );
        }
    }
}


function at_generate_desc() {
    //Disposizioni Generali
$term = get_term_by('name','Piano triennale per la prevenzione della corruzione e della trasparenza','tipologie');
...
$term = get_term_by('name','Titolari di incarichi di collaborazione o consulenza	','tipologie');
...
$term = get_term_by('name','Organismi indipendenti di valutazione, nuclei di valutazione o altri organismi con funzioni analoghe','tipologie');
wp_update_term($term->term_id, 'tipologie', array('description'=>'
...
$term = get_term_by('name','Organi di revisione amministrativa e contabile','tipologie');
wp_update_term($term->term_id, 'tipologie', array('description'=>'Relazioni degli organi di revisione amministrativa e contabile al bilancio di previsione o budget, alle relative variazioni e al conto consuntivo o bilancio di esercizio'));
$term = get_term_by('name','Corte dei Conti','tipologie');
wp_update_term($term->term_id, 'tipologie', array('description'=>'Tutti i rilievi della Corte dei conti ancorchè non recepiti riguardanti l\'organizzazione e l\'attività delle amministrazioni stesse e dei loro uffici'));
...
$term = get_term_by('name','Prevenzione della Corruzione','tipologie');
